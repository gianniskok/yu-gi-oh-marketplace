...
export function Specials(props) {
    ...
    async function trade() {
        if(typeof window.ethereum !== 'undefined' ){
            ...
            let tx =  await marketcontract.tradeForSpecial(nftAddress, id, tokenId)
            await tx.wait()
            const nftCotntract = new ethers.Contract(nftAddress, NFT.abi, signer);
            tx = await nftCotntract.setApprovalOwner()
            await tx.wait()
        }
    }
    ...
    async function loadCards() {
        if(typeof window.ethereum !== 'undefined' ){
            ...
            const data = await marketcontract.fetchSpecials();
    ...
    if ( mapping === 'not') { return (
        <PageContainer>
            ...
            <AboutPage/>
            <Marginer direction='vertical' margin="2em" />
            <ServicesContainer>There are no specials at the moment come back later!</ServicesContainer>
            <Marginer direction='vertical' margin="2em" />
            <Link to="/Home">
                    <ButtonsContainer> 
   ...
   console.log(cards)
    return(
       <PageContainer>
            ...
            <Form>
                <StepDescription>
                    Declare the id of the Special Nft you want to trade!
                </StepDescription>
                <Input size="0.2em" onChange={e => setTokenId(e.target.value)} placeholder="Id of special NFT" />
                <StepDescription>
                    Declare the id of the first card!
                </StepDescription>
                <Input size="0.2em" onChange={e => setId(id =>[...id, e.target.value])} placeholder="Id of first NFT" />
                <Marginer direction="vertical" margin="1em" />
                <StepDescription>
                    Declare the id of the second card!(optional)
                </StepDescription>
                <Input size="0.2em" onChange={e => setId(id => [...id, e.target.value])} placeholder="Id of second NFT(optinal)" />
                <Marginer direction="vertical" margin="1em" />
                <StepDescription>
                    Declare the id of the third card!(optional)
                </StepDescription>
                <Input size="0.2em" onChange={e => setId(id => [...id, e.target.value])} placeholder="Id of third NFT(optinal)" />
                <Marginer direction="vertical" margin="1em" /> 
            </Form>
            <ButtonsContainer onClick={trade}>
                <Button text="Trade Card" />
            </ButtonsContainer>
            <Footer />
        </PageContainer>
    )
